...
The CLI drives `BlackjackEnv` directly: you see the same observation the agent gets
...
the bet spread — takes millions of rounds; `--rounds`, `--lr`, `--eps-decay-frac` and `--resume` are
the knobs. `--bets 1` trains a flat-betting player if you only care about the playing decisions.
